...
public class BookFileWriter {
	...
	private static final String FILE_NAME = "books-fetched-from-api.txt";
	...
	public void recordBookInFile(Book book) throws IOException {
		...
		try (Writer fileWriter = new FileWriter(FILE_DESTINATION + FILE_NAME, true)){
			fileWriter.write(book.toString() + "\n");
		} catch (IOException e) {
			logger.log(Priority.ERROR, "Problem occurred writing into file : " + FILE_DESTINATION + FILE_NAME);
			e.printStackTrace();
		}  		
	}
}
