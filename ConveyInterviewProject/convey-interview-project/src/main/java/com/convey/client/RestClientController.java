...
import com.convey.client.util.ConveyConfig;
...
public class RestClientController {	
	...
	public Response saveBooksInBulk() {
		try {
			...
			booksForSaving.addAll(new BookDTOFactory().packBooksForSaving());
			int maxNumberOfSavingBooksInOneRequest = ConveyConfig.getProperty(ConveyConfig.MAX_NUMBER_OF_SAVING_BOOKS_IN_ONE_REQUEST);
			if (booksForSaving.size() > maxNumberOfSavingBooksInOneRequest) {
				logger.log(Priority.ERROR, "Cannot save more than " + maxNumberOfSavingBooksInOneRequest + " books at once");
			...
			for (BookDTO bookDtoRequest : booksForSaving) {			
				...
				if (Response.Status.OK.getStatusCode() != response.getStatus()) {
					logger.log(Priority.ERROR, "Error occurred while saving book: " + bookDtoRequest.getIsbn());
					throw new WebApplicationException(Response.Status.INTERNAL_SERVER_ERROR);
				}
			}
		} catch (Exception e) {
